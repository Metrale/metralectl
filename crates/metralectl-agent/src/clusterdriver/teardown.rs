...
async fn a_peer_that_dies_on_startup_fails_the_commit_too() {
    ...
    assert!(err.contains("host-3"), "{err}");
    assert!(
...
async fn a_stop_that_could_not_reach_a_peer_is_reported_and_can_be_retried() {
    ...
    // host-2 becomes unreachable -- the machine, not the container.
    d.kill_for_test(node_id(2));
    ...
    assert!(err.contains("host-2"), "must name the machine: {err}");
    ...
    assert!(
        again.unwrap_err().contains("host-2"),
