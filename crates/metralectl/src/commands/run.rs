...
use metralectl_core::docker::translate::{LaunchContext, translate};
use metralectl_core::hfcache;
use metralectl_core::io::{ProcessRunner, StdProcessRunner};
use metralectl_core::nearest;
...
pub fn run(args: &RunArgs) -> Result<()> {
    ...
    for u in &plan.unmapped {
        ...
        let hint = if let Some(key) = metralectl_core::flags::key_for_flag_spelling(&u.key) {
            format!(". That is the engine's flag name; the setting key is `{key}`")
        } else {
            nearest::did_you_mean(&nearest::nearest(&u.key, metralectl_core::flags::keys()))
        };
        ...
        if args.portable {
            println!(
                ...
                plan.docker.display_portable(
                    Some(&host.home),
                    metralectl_core::platform::home_placeholder()
                )
            );
        } else {
            println!("{}", plan.docker);
    ...
    let port = plan
        ...
        .unwrap_or_else(|| metralectl_core::flags::DEFAULT_SERVE_PORT.to_string());
    ...
    // will refuse, and to `metralectl logs` for a container that no longer exists,
    ...
    println!("logs:     metralectl logs {} --follow", recipe.name);
    println!("stop:     metralectl stop {}", recipe.name);
    Ok(())
}
...
fn cautions(argv: &[String]) -> Vec<String> {
    let accelerator =
        metralectl_agent::telemetry::accelerator_name(&StdProcessRunner).unwrap_or_default();
    ...
    argv.windows(2)
        .filter_map(|w| {
            ...
            metralectl_core::settings::caution(&key, value, &accelerator)
        })
        .collect()
}
...
fn disk_caution_for(hf_cache_dir: &str) -> Option<String> {
    ...
    let free = metralectl_core::platform::free_bytes(path)?;
