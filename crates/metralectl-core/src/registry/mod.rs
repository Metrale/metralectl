...
/// which is precisely the mechanism that let an upstream reserve `metrale` to an
...
impl RegistrySet {
    ...
    pub fn list(&self) -> Vec<RecipeEntry> {
        let mut out: Vec<RecipeEntry> = metrale_recipes_data::all()
            .into_iter()
            .map(|e| RecipeEntry {
    ...
    pub fn resolve(&self, r#ref: &RecipeRef) -> Result<Recipe, ResolveError> {
        match r#ref {
            ...
            // did not, so `@metrale/qwen3.8-27b-nvfp4-unsloh` was refused flatly
    ...
    fn load_builtin(&self, name: &str) -> Option<Result<Recipe, RecipeError>> {
        metrale_recipes_data::get(name).map(|e| {
            Recipe::parse(
                e.name,
                e.yaml,
                Provenance::Builtin {
