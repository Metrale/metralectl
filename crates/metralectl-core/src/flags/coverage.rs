...
/// Every engine flag metralectl will not emit, and why.
///
...
pub static EXCLUDED: &[(&str, &str)] = &[
    // Placement is metralectl's job. It derives the whole rank tuple from the
    ...
    ("no_tui", "metralectl always runs the engine headless; the TUI is never on"),
    ("dangerously_allow_unresolved_kernel_lookups", "serves a model whose dispatch is known to be incomplete"),

    // Model-swapping. metralectl owns the lifecycle of a running model — it is
    ...
    ("ssm_decode_ring_slots", "no recipe needs it; add it when one does"),
    ("marconi_min_tokens", "no recipe needs it; add it when one does"),
    ("prefill_codispatch", "no recipe needs it; add it when one does"),

    // A measurement mode for known-answer tests, not a serving configuration.
    ("hermetic", "a known-answer measurement mode, not a way to serve"),
];
