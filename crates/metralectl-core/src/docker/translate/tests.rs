...
fn a_solo_launch_serves_the_model_with_its_resolved_flags() {
    ...
    assert_eq!(p.docker.name, "metrale-r");
...
fn the_model_cache_is_mounted_from_the_host() {
    let p = plan(&recipe(""), &Placement::Solo);
    assert_eq!(
        p.docker.volumes["/home/operator/.cache/huggingface"],
        "/cache/huggingface"
    );
}
...
fn the_head_rank_serves_the_api_and_carries_coordination_flags() {
    ...
    assert_eq!(p.docker.name, "metrale-r-rank0");
}
...
fn a_worker_rank_is_forced_to_port_zero() {
    ...
    assert_eq!(p.docker.name, "metrale-r-rank1");
}
...
mod argv_purity {
    ...
    fn permitted() -> BTreeSet<String> {
        let mut ok: BTreeSet<String> = METRALE_FLAGS.iter().map(|f| f.flag.to_string()).collect();
        let p = &ROOTLESS_V1;
    ...
    fn wire(v: &ScalarValue) -> metralectl_protocol::settings::SettingValue {
        use metralectl_protocol::settings::SettingValue as S;
        match v {
    ...
    fn a_value_with_shell_syntax_stays_a_single_argument() {
        let req = [(
            ...
            metralectl_protocol::settings::SettingValue::Str("a; rm -rf /".into()),
        )]
        .into_iter()
        .collect();
