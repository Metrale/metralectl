...
fn sample() -> DockerCommand {
    DockerCommand {
        ...
        volumes: [(
            "/home/user/.cache/huggingface".to_string(),
...
fn the_portable_rendering_keeps_host_specifics_symbolic() {
    let portable = sample().display_portable(Some("/home/user"), "$HOME");
    assert!(
...
fn a_dollar_sign_from_recipe_data_is_still_quoted_in_the_portable_form() {
    ...
    let portable = c.display_portable(Some("/home/user"), "$HOME");
    assert!(
